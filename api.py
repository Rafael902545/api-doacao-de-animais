...
def get_animais():
    ...
    for usuario in animais:
        if nome and usuario.get('nome') != nome:
            continue
        if especie and usuario.get('especie') != especie:
            continue
...
def post_animais():
    ...
    novo_id = max((a.get('id', 0) for a in animais), default=0) + 1
    dados['id'] = novo_id
...
def post_usuario():
    ...
    novo_id = max((a.get('id', 0) for a in usuario), default=0) + 1
